...
class Externals_AdminPageFramework_FieldType_textarea extends Externals_AdminPageFramework_FieldType {
...
jQuery( document ).ready( function(){
    ...
    jQuery().registerExternals_AdminPageFrameworkCallbacks( {				
        ...
        added_repeatable_field: function( oCloned, sFieldType, sFieldTagID, iCallType, iSectionIndex, iFieldIndex ) {
            ...
            var _oSettings = jQuery().getExternals_AdminPageFrameworkInputOptions( _oWrap.attr( 'data-id' ) );  

            // Elements
            var _oField              = oCloned.closest( '.externals-field' );
        ...
        stopped_sorting_fields : function( oSortedFields, sFieldType, sFieldsTagID, iCallType ) { 
            ...
            // Update the editor.
            oSortedFields.children( '.externals-field' ).each( function( iIndex ) {
                ...
                var oSettings = jQuery().getExternals_AdminPageFrameworkInputOptions( oWrap.attr( 'data-id' ) );   
        ...
        saved_widget : function( oWidget ) { 
            ...
            jQuery( oWidget ).find( '.externals-field' ).each( function( iIndex ) {
                ...
                // Store the settings.
                jQuery().storeExternals_AdminPageFrameworkInputOptions( 
                    oWrap.attr( 'data-id' ), 
                    { 
...
JAVASCRIPTS;
    ...
    protected function getStyles() {
        return <<<CSSRULES
/* Textarea Field Type */
.externals-field-textarea .externals-input-label-string {
    vertical-align: top;
    margin-top: 2px;
}     
/* Rich Text Editor */
.externals-field-textarea .wp-core-ui.wp-editor-wrap {
    margin-bottom: 0.5em;
}
.externals-field-textarea.externals-field .externals-input-label-container {
    vertical-align: top; 
    float: left;
    clear: both;
} 
/* For meta-boxes */
.postbox .externals-field-textarea .externals-input-label-container {
    width: 100%;
}
/* Prevents the input box sticks out in a meta box, especially in the side meta box. */
.externals-field-textarea textarea {    
    max-width: 100%;
}
CSSRULES;
    ...
    private function _getFieldOutputByLabel($sKey, $sLabel, $aField) {
        ...
        $_sClassSelector = $_bIsArray ? 'externals-field-textarea-multiple-labels' : '';
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $sKey, $aField['label']), "<div class='externals-input-label-container {$_sClassSelector}'>", "<label for='" . $aField['attributes']['id'] . "'>", $this->getElementByLabel($aField['before_input'], $sKey, $aField['label']), $_sLabel ? "<span class='externals-input-label-string' style='min-width:" . $this->sanitizeLength($aField['label_min_width']) . ";'>" . $_sLabel . "</span>" : '', $this->_getEditor($aField), $this->getElementByLabel($aField['after_input'], $sKey, $aField['label']), "</label>", "</div>", $this->getElementByLabel($aField['after_label'], $sKey, $aField['label']),);
        return implode('', $_aOutput);
    }
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='externals-textarea-enabler'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
}