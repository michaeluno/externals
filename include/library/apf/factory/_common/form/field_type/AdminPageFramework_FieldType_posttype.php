...
class Externals_AdminPageFramework_FieldType_checkbox extends Externals_AdminPageFramework_FieldType {
...
jQuery( document ).ready( function(){
    ...
    jQuery( '{$_sClassSelectorSelectAll}' ).each( function(){
        jQuery( this ).before( '<div class=\"select_all_button_container\" onclick=\"jQuery( this ).selectAllExternals_AdminPageFrameworkCheckboxes(); return false;\"><a class=\"select_all_button button button-small\">' + jQuery( this ).data( 'select_all_button' ) + '</a></div>' );
    });            
    jQuery( '{$_sClassSelectorSelectNone}' ).each( function(){
        jQuery( this ).before( '<div class=\"select_none_button_container\" onclick=\"jQuery( this ).deselectAllExternals_AdminPageFrameworkCheckboxes(); return false;\"><a class=\"select_all_button button button-small\">' + jQuery( this ).data( 'select_none_button' ) + '</a></div>' );
    });
});
JAVASCRIPTS;
    ...
    private function _getSelectButtonClassSelectors(array $aFieldTypeSlugs, $sDataAttribute = 'select_all_button') {
        ...
        foreach ($aFieldTypeSlugs as $_sSlug) {
            ...
            $_aClassSelectors[] = '.externals-checkbox-container-' . $_sSlug . "[data-{$sDataAttribute}]";
        }
...
.externals-checkbox-label {
    margin-top: 0.1em;
}
.externals-field input[type='checkbox' ] {
    margin-right: 0.5em;
}     
.externals-field-checkbox .externals-input-label-container {
    padding-right: 1em;
}
.externals-field-checkbox .externals-input-label-string  {
...
.externals-field-posttype .externals-input-label-container {
    padding-right: 1em;
}    
CSSRULES;
