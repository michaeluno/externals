...
class Externals_AdminPageFramework_FieldType_select extends Externals_AdminPageFramework_FieldType {
    ...
    protected function getStyles() {
        return <<<CSSRULES
/* Select Field Type */
.externals-field-select .externals-input-label-container {
    vertical-align: top; 
}
.externals-field-select .externals-input-label-container {
    padding-right: 1em;
...
CSSRULES;
    ...
    protected function getField($aField) {
        $_oSelectInput = new Externals_AdminPageFramework_Input_select($aField['attributes']);
        if ($aField['is_multiple']) {
            $_oSelectInput->setAttribute(array('select', 'multiple'), 'multiple');
        }
        return $aField['before_label'] . "<div class='externals-input-label-container externals-select-label' style='min-width: " . $this->sanitizeLength($aField['label_min_width']) . ";'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . $_oSelectInput->get($aField['label']) . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "</div>" . $aField['after_label'];
    }
}
class Externals_AdminPageFramework_FieldType_size extends Externals_AdminPageFramework_FieldType_select {
    ...
    protected function getStyles() {
        return <<<CSSRULES
/* Size Field Type */
.externals-field-size input {
    text-align: right;
}
.externals-field-size select.size-field-select {
    vertical-align: 0px;     
}
.externals-field-size label {
    width: auto;     
} 
.form-table td fieldset .externals-field-size label {
    display: inline;
}
CSSRULES;
    ...
    protected function _getFieldOutputByLabel($isKey, $sLabel, array $aField) {
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $isKey, $aField['label']), "<div class='externals-input-label-container externals-select-label' style='min-width: " . $this->sanitizeLength($aField['label_min_width']) . ";'>", $this->_getNumberInputPart($aField, $_aBaseAttributes, $isKey, $aField['label']), $this->_getUnitSelectInput($aField, $_aBaseAttributes, $isKey, $aField['label']), "</div>", $this->getElementByLabel($aField['after_label'], $isKey, $aField['label']));
        return implode('', $_aOutput);
    }
    private function _getNumberInputPart(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_sLabel = $this->getElementByLabel($aField['label'], $isKey, $aField['label']);
        return "<label " . $this->getAttributes($_aSizeLabelAttributes) . ">" . $this->getElement($aField, $bMultiLabels ? array('before_label', $isKey, 'size') : array('before_label', 'size')) . ($aField['label'] && !$aField['repeatable'] ? "<span class='externals-input-label-string' style='min-width:" . $this->sanitizeLength($aField['label_min_width']) . ";'>" . $_sLabel . "</span>" : "") . "<input " . $this->getAttributes($_aSizeAttributes) . " />" . $this->getElement($aField, $bMultiLabels ? array('after_input', $isKey, 'size') : array('after_input', 'size')) . "</label>";
    }
    private function _getUnitSelectInput(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        $_oUnitInput = new Externals_AdminPageFramework_Input_select($_aUnitAttributes + array('select' => $_aUnitAttributes));
