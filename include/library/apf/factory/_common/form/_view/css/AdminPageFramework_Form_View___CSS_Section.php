<?php 
/**
	Admin Page Framework v3.7.10b03 by Michael Uno 
	Generated by PHP Class Files Script Generator <https://github.com/michaeluno/PHP-Class-Files-Script-Generator>
	<http://en.michaeluno.jp/externals>
	Copyright (c) 2013-2016, Michael Uno; Licensed under MIT <http://opensource.org/licenses/MIT> */
class Externals_AdminPageFramework_Form_View___CSS_Section extends Externals_AdminPageFramework_Form_View___CSS_Base {
    protected function _get() {
        return $this->_getFormSectionRules();
    }
    private function _getFormSectionRules() {
        return <<<CSSRULES
/* Section Table */
.externals-section .form-table {
    margin-top: 0;
}
.externals-section .form-table td label {
   display: inline;  /* adjusts the horizontal alignment with the th element */
}
/* Section Tabs */
.externals-section-tabs-contents {
    margin-top: 1em;
}
.externals-section-tabs { /* The section tabs' container */
    margin: 0;
}
.externals-tab-content { /* each section including sub-sections of repeatable fields */
    padding: 0.5em 2em 1.5em 2em;
    margin: 0;
    border-style: solid;
    border-width: 1px;
    border-color: #dfdfdf;
    background-color: #fdfdfd;     
}
.externals-section-tab {
    background-color: transparent;
    vertical-align: bottom; /* for Firefox */
    margin-bottom: -1px;    /* For WP 3.4.4+ */
    margin-left: 0px;
    margin-right: 0.5em;
    background-color: #F1F1F1;
    font-weight: normal;
}
.externals-section-tab:hover {
    background-color: #F8F8F8;
}
.externals-section-tab.active {
    background-color: #fdfdfd;     
}
/* Tab title */
.externals-section-tab h4 {
    margin: 0;
    /* padding: 8px 14px 10px; */
    padding: 0.4em 0.8em;
    font-size: 1.12em;
    vertical-align: middle;
    white-space: nowrap;
    display:inline-block;
    font-weight: normal;
}
.externals-section-tab.nav-tab {
    /* padding: 0; */
    padding: 0.2em 0.4em;
}
.externals-section-tab.nav-tab a {
    text-decoration: none;
    color: #464646;
    vertical-align: inherit; /* for Firefox - without this tiny dots appear */
    outline: 0; /* for FireFox - remove dotted outline */
}        
.externals-section-tab.nav-tab a:focus { 
    /* For FireFox - remove dotted outline when a switchable tab is activated */
    box-shadow: none;
}
.externals-section-tab.nav-tab.active a {
    color: #000;
}
.externals-content ul.externals-section-tabs > li.externals-section-tab {    
    /* Do not show bullets in section tabs */
    list-style-type: none;
    /* For WordPress 4.4, make sure to attach the tab to the container */
    margin: -4px 4px -1px 0;
}
/* Repeatable Sections */
.externals-repeatable-section-buttons {
    float: right;
    clear: right;
    margin-top: 1em;
}
/* Section Caption */
.externals-section-caption {
    text-align: left;
    margin: 0;
}
/* Section Title */
.externals-section .externals-section-title {
    /* background: none; */               /* @todo examine what this is for. @deprecated 3.4.0 for repeatable collapsible section titles */
    /* -webkit-box-shadow: none; */       /* @todo examine what this is for. @deprecated 3.4.0 for repeatable collapsible section titles */
    /* _box-shadow: none; */              /* @todo examine what this is for. @deprecated 3.4.0 for repeatable collapsible section titles */
}

/* Sortable Sections */
.externals-sections.sortable-section > .externals-section {
    /* padding: 1em 2.5em 1.5em 2.5em; */
    padding: 1em 1.8em 1em 2.6em;
    
}

/* Sortable Collapsible Sections */
.externals-sections.sortable-section > .externals-section.is_subsection_collapsible {
    display: block; 
    float: none;
    border: 0px;
    padding: 0;
    background: transparent;
}
/* Sortable Tabbed Sections */
.externals-sections.sortable-section > .externals-tab-content {
    display: block; 
    float: none;
    border: 0px;    

    padding: 0.5em 2em 1.5em 2em;
    margin: 0;
    border-style: solid;
    border-width: 1px;
    border-color: #dfdfdf;
    background-color: #fdfdfd;      
}

.externals-sections.sortable-section > .externals-section {
    margin-bottom: 1em;
}
.externals-section {
    /* gives a bottom margin between sections. This helps for the debug info in each sectionset and collapsible sections. */
    margin-bottom: 1em; 
}            
.externals-sectionset {
    margin-bottom: 1em; 
    display:inline-block;
    width:100%;
}            
/* Nested sections */
.externals-section > .externals-sectionset {
    margin-left: 2em;
}

CSSRULES;
        
    }
}