 ...
 * @package     Externals
 * @since        1
 * @since       1       Extends `Externals_WPUtility`.
 */
final class Externals_DOM extends Externals_PluginUtility {
    ...
    function __construct() {
        ...
        $this->sHTMLCachePrefix = Externals_Registry::TRANSIENT_PREFIX . "_HTML_";
    ...
    public function getHTML( $sURL, $bUseFileGetContents=false ) {
        ...
        if ( $bUseFileGetContents ) {
            $_oHTML = new Externals_HTTPClient_FileGetContents( $sURL );
            return $_oHTML->get();
        }
        $_oHTML = new Externals_HTTPClient( $sURL );
        return $_oHTML->get();    
